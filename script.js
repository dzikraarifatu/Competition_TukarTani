...
const rewardsList = [
  ...
  { id:9,  name:'Silase Jerami Padi',   desc:'1 Kg — Bank Silase', cost:80,  icon:'fa-wheat-awn', colorClass:'bg-amber-100 dark:bg-amber-900/30 text-amber-600',    isUmkm:false },
  { id:10, name:'Silase Batang Jagung', desc:'1 Kg — Bank Silase', cost:70,  icon:'fa-seedling',  colorClass:'bg-orange-100 dark:bg-orange-900/30 text-orange-600',  isUmkm:false },
  { id:11, name:'Silase Batang Pisang', desc:'1 Kg — Bank Silase', cost:60,  icon:'fa-tree',      colorClass:'bg-green-100 dark:bg-green-900/30 text-green-600',      isUmkm:false },
  { id:12, name:'Pupuk Organik Sapi',   desc:'1 Kg — Rumah Kompos', cost:90,  icon:'fa-leaf',      colorClass:'bg-teal-100 dark:bg-teal-900/30 text-teal-600',        isUmkm:false },
  { id:13, name:'Pupuk Organik Kambing',desc:'1 Kg — Rumah Kompos', cost:100, icon:'fa-seedling',  colorClass:'bg-amber-100 dark:bg-amber-900/30 text-amber-700',     isUmkm:false },
];
...
function updateUIWithUserData() {
  ...
  const ca = document.getElementById('profil-carbon-amount');
  if (ca) {
    const totalKg = parseFloat(currentUser.carbonReduction||0);
    const display = totalKg >= 1000 ? `${(totalKg/1000).toFixed(2)} Ton` : `${totalKg.toFixed(1)} kg`;
    ca.innerHTML = `<i class="fa-solid fa-arrow-trend-down"></i> ${display} CO₂e`;
  }
  ...
  if (bcarb) {
    const totalKg = parseFloat(currentUser.carbonReduction||0);
    const display = totalKg >= 1000 ? (totalKg/1000).toFixed(2) : totalKg.toFixed(1);
    const unit = totalKg >= 1000 ? 'Ton CO₂e' : 'kg CO₂e';
    bcarb.innerHTML = `${display}<span class="text-lg font-bold text-teal-100 ml-1">${unit}</span>`;
  }
...
const BUMDES_FLAT_RATE = 150; // tarif flat per penggunaan armada (sesuai narasi esai)
  ...
  if (exPoin) {
    if (potonganBumdes > 0) {
      exPoin.innerHTML = `
        ...
        <span class="block text-red-500 text-xs font-bold">- ${potonganBumdes} Poin (Tarif Flat Armada BUMDes)</span>
  ...
  if (ntcBreakdownEl) {
    ...
    ntcBreakdownEl.innerHTML = `
      <div class="text-[10px] font-bold text-slate-500 dark:text-slate-400 space-y-0.5">
        ...
        ${potonganBumdes?`<p>🚛 Potongan BUMDes: <b class="text-red-500">Tarif Flat ${BUMDES_FLAT_RATE} Poin</b></p>`:''}
      </div>`;
  }
  ...
  <div id="modal-bank-silase" class="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[4500] hidden flex-col items-center justify-center p-6 opacity-0 transition-opacity duration-300">
    <div id="bank-silase-content" class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl max-w-lg w-full transform scale-90 transition-transform duration-300 max-h-[92vh] overflow-y-auto no-scrollbar">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <button onclick="closeBankSilase(); switchTab('katalog'); setTimeout(()=>showToast('Pilih Silase di katalog untuk ditukar!','fa-wheat-awn','text-amber-400'),400);" class="flex-1 bg-gradient-to-r from-amber-500 to-amber-600 text-white font-extrabold py-3.5 rounded-2xl shadow-lg shadow-amber-500/30 transition-all text-sm flex items-center justify-center gap-2">
          🌾 Tukar Poin dengan Silase
        </button>
      </div>
    </div>
  </div>`;
...
// ── GAP: RUMAH KOMPOS ─────────────────────────────────────────
function openRumahKompos() {
  const existing = document.getElementById('modal-rumah-kompos');
  if (existing) {
    existing.classList.remove('hidden'); existing.classList.add('flex');
    setTimeout(()=>{ existing.classList.remove('opacity-0'); document.getElementById('rumah-kompos-content')?.classList.remove('scale-90'); },10);
    return;
  }

  const modalHtml = `
  <div id="modal-rumah-kompos" class="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[4500] hidden flex-col items-center justify-center p-6 opacity-0 transition-opacity duration-300">
    <div id="rumah-kompos-content" class="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-2xl max-w-lg w-full transform scale-90 transition-transform duration-300 max-h-[92vh] overflow-y-auto no-scrollbar">

      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
          <div class="w-12 h-12 rounded-2xl bg-amber-100 dark:bg-amber-900/30 text-amber-700 flex items-center justify-center text-2xl">🍂</div>
          <div>
            <h2 class="text-xl font-extrabold text-slate-900 dark:text-white">Rumah Kompos</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">Pengolahan Kotoran Ternak Menjadi Pupuk Organik</p>
          </div>
        </div>
        <button onclick="closeRumahKompos()" class="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 flex items-center justify-center hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
          <i class="fa-solid fa-xmark"></i></button>
      </div>
      ...
      <div class="bg-amber-50 dark:bg-amber-900/10 border-2 border-amber-100 dark:border-amber-800/50 p-4 rounded-2xl mb-6">
        <p class="text-xs font-extrabold text-amber-700 dark:text-amber-400 uppercase tracking-widest mb-2">🔴 Masalah</p>
        <p class="text-sm font-medium text-amber-900 dark:text-amber-200 leading-relaxed">
          Kotoran sapi tidak bisa langsung ditabur ke tanaman karena masih panas dan mengandung patogen. Butuh pengomposan 3–4 minggu untuk menjadi pupuk organik matang.
        </p>
      </div>

      <p class="text-sm font-extrabold text-slate-900 dark:text-white mb-4">Alur Pengomposan Rumah Kompos:</p>
      <div class="space-y-3 mb-6">
        ${[
          {step:'1',icon:'fa-trowel',color:'bg-brown-100 dark:bg-brown-900/30 text-brown-600',title:'Pengumpulan Kotoran',desc:'Peternak menyerahkan kotoran sapi ke Drop Point Balai Desa. Kelompok Tani mencatat berat dan kondisi.'},
          {step:'2',icon:'fa-layer-group',color:'bg-amber-100 dark:bg-amber-900/30 text-amber-600',title:'Pencampuran',desc:'Kotoran dicampur sekam (50:50) + kapur 5% + mikroorganisme lokal (MOL).'},
          {step:'3',icon:'fa-hourglass-half',color:'bg-orange-100 dark:bg-orange-900/30 text-orange-600',title:'Pengomposan Aerob',desc:'Tumpukan ditutup terpal, dibalik 3 hari sekali selama 3–4 minggu. Suhu naik lalu turun = tanda matang.'},
          {step:'4',icon:'fa-seedling',color:'bg-green-100 dark:bg-green-900/30 text-green-600',title:'Pupuk Organik Matang',desc:'Pupuk siap diaplikasikan ke lahan. Bisa juga difermentasi menjadi biogas skala rumah tangga (Fidela et al., 2024).'},
        ].map(s=>`
        <div class="flex gap-4 items-start">
          <div class="w-10 h-10 rounded-2xl ${s.color} flex items-center justify-center shrink-0 font-extrabold text-lg">
            <i class="fa-solid ${s.icon} text-sm"></i>
      ...
      <div class="bg-slate-50 dark:bg-slate-950 rounded-2xl p-4 mb-6 border border-slate-100 dark:border-slate-800">
        <p class="text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">Stok Pupuk Organik Desa</p>
        <div class="space-y-2">
          ${[
            {nama:'Pupuk Organik Sapi',stok:'850 Kg',harga:'90 Poin/Kg'},
            {nama:'Pupuk Organik Kambing',stok:'420 Kg',harga:'100 Poin/Kg'},
            {nama:'Pupuk Organik Ayam',stok:'310 Kg',harga:'120 Poin/Kg'},
          ].map(s=>`
          <div class="flex items-center justify-between gap-2">
            <span class="text-sm font-bold text-slate-800 dark:text-slate-200">${s.nama}</span>
            <div class="text-right">
      ...
      <div class="grid grid-cols-2 gap-3">
        <button onclick="closeRumahKompos()" class="flex-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-extrabold py-3.5 rounded-2xl transition-all hover:bg-slate-200 dark:hover:bg-slate-700 text-sm">
          Tutup
        </button>
        <button onclick="closeRumahKompos(); switchTab('katalog'); setTimeout(()=>showToast('Pilih Pupuk Organik di katalog!','fa-leaf','text-green-400'),400);" class="flex-1 bg-gradient-to-r from-amber-500 to-amber-600 text-white font-extrabold py-3.5 rounded-2xl shadow-lg shadow-amber-500/30 transition-all text-sm flex items-center justify-center gap-2">
          <i class="fa-solid fa-coins"></i> Tukar Poin dengan Pupuk
        </button>
      </div>
    </div>
  </div>`;
  ...
  document.body.insertAdjacentHTML('beforeend', modalHtml);
  const modal = document.getElementById('modal-rumah-kompos');
  modal.classList.remove('hidden'); modal.classList.add('flex');
  setTimeout(()=>{ modal.classList.remove('opacity-0'); document.getElementById('rumah-kompos-content')?.classList.remove('scale-90'); },10);
}

function closeRumahKompos() {
  const modal = document.getElementById('modal-rumah-kompos');
  modal?.classList.add('opacity-0');
  document.getElementById('rumah-kompos-content')?.classList.add('scale-90');
  setTimeout(()=>{ modal?.classList.add('hidden'); modal?.classList.remove('flex'); },300);
}
...
function startChatWithMitra(namaLabelOrBtn, iconType, jarak) {
  // Bisa dipanggil dari popup Leaflet (HTMLElement) atau dari JS langsung (string)
  let namaLabel, iconT, jarakT;
  if (namaLabelOrBtn instanceof HTMLElement) {
    namaLabel = namaLabelOrBtn.getAttribute('data-label');
    iconT     = namaLabelOrBtn.getAttribute('data-type');
    jarakT    = namaLabelOrBtn.getAttribute('data-jarak');
  } else {
    namaLabel = namaLabelOrBtn; iconT = iconType; jarakT = jarak;
  }
  const colorMap = {'cow':'eab308','wheat-awn':'22c55e','crow':'f97316','seedling':'10b981'};
  activeMitra = { name: namaLabel, jarak: jarakT||'2.4 km', avatarColor: colorMap[iconT]||'10b981' };
  _resetAndOpenChat(activeMitra);
}
        ...
        <label class="cursor-pointer">
          ...
          <div class="p-4 border-2 border-slate-200 dark:border-slate-800 rounded-2xl flex items-center gap-3 peer-checked:border-blue-500 peer-checked:bg-blue-50 dark:peer-checked:bg-blue-900/20 transition-all">
            ...
            <div><p class="text-sm font-extrabold text-slate-900 dark:text-white">🚛 Armada BUMDes</p>
              <p class="text-xs text-slate-500 dark:text-slate-400">Layanan antar-jemput desa, tarif flat 150 Poin per penggunaan</p></div>
          </div>
        </label>
        <label class="cursor-pointer">
...
function finalizeDeal(rating) {
  ...
  // Laporan karbon per transaksi
  const sl = document.getElementById('success-limbah');
  const sc = document.getElementById('success-carbon');
  if (sl) sl.innerText = `${vol} kg`;
  if (sc) sc.innerText = `${totalCarbon.toFixed(1)} kg CO₂e`;
    ...
    const marker=L.marker([p.lat,p.lng],{icon:L.divIcon({html:iconHtml,className:'',iconSize:[44,55],iconAnchor:[22,55],popupAnchor:[0,-55]})})
      ...
      .bindPopup(`<div style="text-align:center;min-width:170px;padding:6px 4px;">
        ...
        <span style="font-size:10px;color:#64748b;">📍 ${p.jarak} dari Anda</span><br>
          <button onclick="window.startChatWithMitra(this)" data-label="${p.label}" data-type="${p.type}" data-jarak="${p.jarak}" style="margin-top:8px;background:#10b981;color:white;border:none;padding:7px 18px;border-radius:10px;font-size:11px;font-weight:700;cursor:pointer;width:100%;pointer-events:auto;">
...
function initMap() {
  ...
  L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}',{attribution:'© Esri © OpenStreetMap contributors',maxZoom:19}).addTo(mapInstance);
...
function initMatchMap() {
  ...
  L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}',{attribution:'',maxZoom:19}).addTo(matchMapInstance);
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if (localStorage.getItem('isLoggedIn')==='true' && loadCurrentUser()) {
    ...
    if (splash) {
      setTimeout(()=>{ splash.classList.add('splash-exit'); setTimeout(()=>splash.style.display='none',650); },2000);
    }
    document.getElementById('auth-view')?.classList.add('hidden');
    document.getElementById('app-view').classList.remove('hidden');
    document.getElementById('app-view').classList.add('flex');
